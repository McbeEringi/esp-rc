...
const char html[] PROGMEM=R"rawliteral(
	...
	<html lang="en" dir="ltr">
		...
		<body>
			<style>
				:root{transition:.5s;--box:min(100vmin,480px);--cir:0.8;--dot:0.1;}@media(prefers-color-scheme:dark){:root{background-color:#222;color:#fff;}}body{margin:0;}
				#stick{position:relative;width:calc(var(--box) * var(--cir));height:calc(var(--box) * var(--cir));box-shadow:0 0 0 calc(var(--box) * calc(calc(var(--cir) * var(--dot)) / 2)) #8882;border-radius:50%;background:#8884;margin:calc(var(--box) * calc(calc(1 - var(--cir)) / 2)) auto;user-select:none;-webkit-user-select:none;}
